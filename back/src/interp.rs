            ...
            match rx.recv_timeout(Duration::from_nanos(1)) {
                Ok(dc) => match dc {
                    ...
                    DebugCommands::Diassemble((addr, arm_mode)) => {
                        let paddr = self.cpu.translate(TLBReq { vaddr: VirtAddr(addr), kind: Access::Debug });
                        if paddr.is_err() {
                            let s = format!("Disassembly failed - translation failure: {addr:04x} {:?}", paddr);
                            tx.send(DebugCommands::Data(s.into_bytes().into_boxed_slice())).unwrap();
                        }
                        let paddr = paddr.unwrap();
                        let bus = self.cpu.bus.read();
                        let dissassemble_res = if arm_mode {
                            let opcd = bus.read32(paddr);
                            drop(bus);
                            match opcd {
                                Ok(opcd) => crate::bits::disassembly::disassmble_arm(opcd, addr),
                                Err(err) => Err(err),
                            }
                        } else {
                            let opcd = bus.read16(paddr);
                            drop(bus);
                            match opcd {
                                Ok(opcd) => crate::bits::disassembly::disassmble_thumb(opcd, addr),
                                Err(err) => Err(err),
                            }
                        };
